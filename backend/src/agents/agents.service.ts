...
import { cloudAgentPrompts } from './prompts/cloud';
import { localAgentPrompts } from './prompts/local';
...
export class AgentsService {
  ...
  private get promptSet() {
    return ['openai', 'cloud'].includes(this.provider) ? cloudAgentPrompts : localAgentPrompts;
  }
  ...
  async explain(message: string, context?: string, userId?: string) {
    ...
    const system = this.promptSet.explainSystem;
  ...
  async describeImage(payload: { imageBase64: string; note?: string }) {
    ...
    const prompts = this.promptSet;
    const prompt = [
      new SystemMessage(prompts.visionSystem),
      new HumanMessage({
        content: [
  ...
  async generateIngredientEstimate(payload: {
    ...
    const prompt: { role: 'system' | 'user'; content: string }[] = [
      {
        role: 'system',
        content: prompts.ingredientSystem,
      },
      {
        role: 'user',
  ...
  async generateDayPlanWithCoachLLM(payload: {
    ...
    const prompt: { role: 'system' | 'user'; content: string }[] = [
      {
        role: 'system',
        content: prompts.dayCoachSystem,
      },
      {
        role: 'user',
        content: JSON.stringify({
