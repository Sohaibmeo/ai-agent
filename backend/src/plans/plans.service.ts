...
export type GenerateWorkflowDayInput = {
  planId: string;
  userId: string;
...
export class PlansService {
  ...
  async createWorkflowPlanDraft(userId: string, weekStartDate: string) {
    ...
    this.logger.log(`workflow plan draft persisted id=${savedPlan.id} user=${userId}`);
    return savedPlan;
  }

  async generateWorkflowDay(input: GenerateWorkflowDayInput) {
    const totalDays = 7;
    const plan = await this.weeklyPlanRepo.findOne({
    ...
    if (plan.user?.id && plan.user.id !== input.userId) {
      throw new Error('Forbidden: plan does not belong to user');
    }
    ...
    this.logger.log(`workflow generated plan=${plan.id} day=${input.dayIndex} meals=${mealsCreated}`);
    return {
      planId: plan.id,
      dayIndex: input.dayIndex,
      status: 'generated',
  ...
  async finalizeWorkflowPlan(planId: string, userId: string) {
    const plan = await this.weeklyPlanRepo.findOne({ where: { id: planId }, relations: ['user'] });
    if (!plan) throw new NotFoundException('Weekly plan not found');
    if (plan.user?.id && plan.user.id !== userId) {
      throw new Error('Forbidden: plan does not belong to user');
    }
    ...
    this.logger.log(`workflow finalized plan=${planId}`);
    return {
      planId,
      status: 'finalized',
    };
